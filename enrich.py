...
# ── Case name keywords: flag if these appear in the case title ────────────────
# Matched as whole words against case_name. Fires independently of other criteria.
CASE_NAME_KEYWORDS = [
    "artificial intelligence", "data breach",
]
# These short keywords need word-boundary matching to avoid false positives
CASE_NAME_KEYWORDS_SHORT = [
    "ai",
]
...
def load_watchlist() -> set[str]:
    """Load case numbers from watchlist.txt (one per line, # for comments)."""
    ...
    with open(WATCHLIST_FILE) as f:
        for line in f:
            ...
            if line and not line.startswith("#"):
                cases.add(normalize_case_number(line))
    return cases
...
def score_relevance(row: dict, cl_data: dict, watchlist: set[str]) -> tuple[bool, list[str]]:
    """Return (is_relevant, [reasons]) based on CourtListener data and watchlist."""
    reasons = []

    # 0. Watchlist — fires regardless of CourtListener data
    if normalize_case_number(row["case_number"]) in watchlist:
        reasons.append("Watchlist")

    if not cl_data or cl_data.get("_not_found"):
        return len(reasons) > 0, reasons

    # 1. Case name keyword match (e.g., "AI", "data breach" in title)
    case_name_lower = cl_data.get("case_name", "").lower()
    for kw in CASE_NAME_KEYWORDS:
        if kw in case_name_lower:
            reasons.append(f"Title: '{kw}'")
            break
    for kw in CASE_NAME_KEYWORDS_SHORT:
        if _word_match(kw, case_name_lower):
            reasons.append(f"Title: '{kw}'")
            break

    # 2. Nature-of-suit code (narrow list — only specific tech/privacy codes)
    nos = cl_data.get("nos_code", "")
    if nos in RELEVANT_NOS_CODES:
        reasons.append(f"NOS {nos}: {RELEVANT_NOS_CODES[nos]}")

    # 3. Named tech company in case name (whole-word match to avoid substrings)
    ...
    # 4. Cause keywords — strong triggers fire regardless; weak only if tech party matched
...
def run(relevant_days: int = 7):
    ...
    if watchlist:
        logger.info(f"Loaded {len(watchlist)} case(s) from watchlist")
    ...
    for row in schedule:
        ...
        is_relevant, reasons = score_relevance(row, cl, watchlist)
