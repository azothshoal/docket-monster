...
def main():
    args = sys.argv[1:]
    arg_set = set(args)
    # DOCKET_MONSTER_NO_SCRAPE forces no-scrape regardless of flags — used for
    # demos/test runs so the court site is never hit accidentally.
    env_no_scrape = os.getenv("DOCKET_MONSTER_NO_SCRAPE", "").strip().lower() in ("1", "true", "yes")
    skip_scrape = env_no_scrape or "--no-scrape" in arg_set or "--dry-run" in arg_set
