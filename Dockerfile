...
# Copies all .py plus any *.txt (requirements.txt, watchlist.example.txt, and
# watchlist.txt if present locally). watchlist.txt is git-ignored, so a clean
# clone still builds — it just runs without a watchlist.
COPY *.py *.txt ./
